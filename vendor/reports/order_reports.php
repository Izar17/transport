...
$week = isset($_GET['week']) ? $_GET['week'] : date("o-\WW");
                ...
                <div class="callout callout-primary shadow rounded-0">
                        ...
                        <div class="row align-items-end">
                            ...
                            <div class="col-lg-3 col-md-4 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date" id="date" value="<?= $date ?>" class="form-control rounded-0" required>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-4 col-sm-12">
                                <div class="form-group">
                    ...
                    <table class="table table-bordered table-stripped">
                        ...
                        <tbody>
                            ...
                            $i = 1;
                            $total = 0;
                            $orders = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ',COALESCE(c.middlename,'')) as client from `order_list` o inner join client_list c on o.client_id = c.id where o.vendor_id = '{$_settings->userdata('id')}' and (date_format(o.date_created,'%Y-%m') = '{$month}' or date_format(o.date_created,'%Y-%u') = '{$week}' or date_format(o.date_created,'%Y-%m-%d') = '{$date}') order by unix_timestamp(o.date_created) desc ");
...
<div class="d-flex align-items-center">
    ...
    <div class="col-auto flex-shrink-1 flex-grow-1 px-4">
        ...
        <h5 class="text-center m-0"><?= date("o-\WW", strtotime($week)) ?></h5>
        <h5 class="text-center m-0">For the Day of</h5>
        <h5 class="text-center m-0"><?= date("F d, Y", strtotime($date)) ?></h5>
    </div>
</div>
<hr>
</noscript>
<script>
    $(function(){
        ...
        $('#print').click(function(){
            ...
            head.find('title').text("Orders Report - Print View")
            el.append(head)
            el.append(header)
            el.append(p)
