...
Class Master extends DBConnection {
	...
	function place_order(){
		...
		while($vrow = $vendors->fetch_assoc()):
			...
			$data = "('{$ref_code}','{$this->settings->userdata('id')}','{$vrow['id']}','{$this->conn->real_escape_string($delivery_address)}','{$this->conn->real_escape_string($reference_no)}')";
			$sql = "INSERT INTO `order_list` (`code`,`client_id`,`vendor_id`,`delivery_address`,`reference_no`) VALUES {$data}";
			$save = $this->conn->query($sql);
			if($save){
